...
if __name__=="__main__":
    ...
    result=dict(status=qualified["status"],totals=totals,components=rows,
        cpu_over_phase61={str(t):controls[t]["regular_over_candidate"] for t in controls},
        cpu_reports={str(t):controls[t]["regular_cpu_summary_reports"] for t in controls},
        kernel_controls=kernels,
        scope="Six balanced Phase60/61 pairs; two fresh regular CPU runs each at 1 and 48 Numba threads, separately measured; BLAS/OpenMP one thread. Not a strongest-possible CPU rewrite. Exact output contracts unchanged. Hybrid improvements, not GPU-only attribution.")
    ...
    lines=["# Phase 61: complete CPU/accelerated comparison","",result["scope"],"",
    ...
    lines += ["","## All 34 model steps","","| Step | CPU 1 (s) | CPU 48 (s) | Phase 60 (s) | Phase 61 (s) | CPU 48 / Phase 61 |","|---|---:|---:|---:|---:|---:|"]
    for row in rows:
    ...
    print(json.dumps({"totals":totals,"cpu_over_phase61":result["cpu_over_phase61"]},indent=2))
