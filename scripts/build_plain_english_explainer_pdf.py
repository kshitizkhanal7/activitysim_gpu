...
def cover_story():
    ...
    return [
        ...
        Spacer(1, 0.25 * inch),
        Paragraph(
            "Read left to right: early phases establish strict CPU answers, generated CUDA, controlled random-number tests, and complete-output verification. Later phases keep skims and trip state on the GPU, remove dense transfers, and move the full 1,454-zone destination sampler onto CUDA. Phase 42 turns exact arithmetic rules into a hash-addressed compiler. Phases 43-49 remove repeated random-state rows, generic choice plumbing, and the inter-stage mode-logsum round trip. Phases 50-53 eliminate dense destination inputs and repeated owner rows, fuse a checked expression program, prewarm a persistent four-row service, and intercept compact owner facts before pandas expands them. Phase 54 links sampling to logsums with versioned GPU buffer leases, generates legacy-compatible controlled normal draws and taxi/TNC wait tables on CUDA, and reduces packet preparation 4.252x. Three complete public-benchmark runs produce a 133.714-second median, 1.536x faster than the separately measured 205.4-second regular ActivitySim CPU median. All modeled decisions remain exact; assumptions, bounded diagnostics, failures, variance, CPU/GPU claim boundaries, memory cost, and portability limits are explained inside.",
            STYLES["small"],
        ),
        Spacer(1, 0.9 * inch),
