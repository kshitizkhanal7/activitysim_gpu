...
def cover_story():
    ...
    return [
        ...
        Spacer(1, 0.25 * inch),
        Paragraph(
            "Read left to right: early phases establish strict CPU answers, generated CUDA, controlled random-number tests, and complete-output verification. Later phases keep skims and trip state on the GPU, remove dense transfers, and move the full 1,454-zone destination sampler onto CUDA. Phase 42 turns exact arithmetic rules into a hash-addressed compiler. Phases 43 and 44 remove repeated random-state rows and generic pandas choice plumbing. Phase 45 applies one reviewed CUDA sampler to five destination families and 274 million utility cells. Phase 46 adds a persistent service and exact keyed MT19937 generation on GPU. Phase 47 compiles all 4,696,676 final sampled-alternative utilities to strict CUDA. Phase 48 keeps normalization, probability choice, compact logsum reduction, and continued random state on the GPU. Phase 49 removes the inter-stage mode-logsum round trip. Phase 50 bypasses the dense pandas logsum preprocessor, generates its exact 10-float, 31-integer, six-skim-group ABI on CUDA, avoids 1.896 GB of input transfer, and improves the five destination components from 28.4 to 16.9 seconds. In a separate three-pair comparison, the complete latest system improves regular ActivitySim from 209.300 to 134.118 seconds, a 1.561x speedup. All modeled decision cells remain exact; assumptions, bounded diagnostics, failures, variance, CPU/GPU claim boundaries, and portability limits are explained inside.",
            STYLES["small"],
        ),
        Spacer(1, 0.9 * inch),
