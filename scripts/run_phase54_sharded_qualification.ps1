param(
    ...
    [ValidatePattern("^[A-Za-z0-9-]+$")][string]$RunTag = "p54final"
)
...
foreach ($required in @($python, $project, $data, $overlay, $sharrow, $reference, $checkpointSource, $runner)) {
    if (-not (Test-Path -LiteralPath $required)) { throw "Missing Phase 54 input: $required" }
}
...
for ($trial = 1; $trial -le $Repetitions; $trial++) {
    $preReport = Join-Path $repo "benchmark-results\phase54-$RunTag-pre-gpu-$trial.json"
    $postReport = Join-Path $repo "benchmark-results\phase54-$RunTag-post-gpu-$trial.json"
    $preOutput = Join-Path $project "o-$RunTag-pre-50000-$trial"
    foreach ($shard in @("pre", "post")) {
        $report = if ($shard -eq "pre") { $preReport } else { $postReport }
        if (Test-Path -LiteralPath $report) { throw "Refusing to overwrite $report" }
        $kernel = Join-Path $repo "benchmark-results\phase54-$RunTag-$shard-kernels-$trial"
        New-Item -ItemType Directory -Path (Join-Path $kernel "mode") -Force | Out-Null
        $checkpoint = Join-Path $repo "benchmark-results\phase54-$RunTag-$shard-checkpoint-$trial.json"
        $output = if ($shard -eq "pre") { $preOutput } else { $postOutput }
        ...
        if ($shard -eq "pre") {
            $arguments += @("--stop-after-model", "workplace_location")
        } else {
            $arguments += @(
        ...
        if ($process.ExitCode -ne 0) {
            throw "Phase 54 $shard shard $trial failed; see $stderr"
        }
    }
}

Write-Output "Phase 54 shards completed: $Repetitions matched pairs"
