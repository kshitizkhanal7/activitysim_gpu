param(
    ...
    [ValidateSet("phase17", "phase34", "phase35", "phase36", "phase37", "activitysim")][string]$Baseline = "phase17",
    [ValidateSet(32, 33, 34, 35, 36, 37, 38)][int]$CandidatePhase = 32,
    [switch]$Resume
)
...
for ($trial = 1; $trial -le $Repetitions; $trial++) {
    ...
    $baselineComplete = (
        ...
        ($Baseline -notin @("phase34", "phase35", "phase36", "phase37") -or (Test-Path -LiteralPath $baselineReport))
    )
    $candidateComplete = (
    ...
    if ($baselineComplete) {
        ...
        if ($Baseline -in @("phase34", "phase35", "phase36", "phase37")) {
            $env:CHOICEFORGE_STRICT_CUDA_CANDIDATE = "1"
            $env:CHOICEFORGE_STRICT_CUDA_MODE_CHOICE = "1"
            $baselineArguments = @(
...
foreach ($modelName in $runs[0].baseline_component_seconds.Keys) {
    ...
    $gpuRole = switch ($modelName) {
        ...
        "trip_destination" { if ($CandidatePhase -ge 38) { "Phase38 normalized resident trip state" } elseif ($CandidatePhase -ge 37) { "Phase37 fused raw-state utility" } elseif ($CandidatePhase -ge 36) { "Phase36 device-generated trip ABI" } elseif ($CandidatePhase -ge 35) { "Phase35 native raw-trip GPU ABI" } else { "Phase17 generated GPU retained" } }
...
$baselineLabel = if ($Baseline -eq "activitysim") {
    ...
    if ($Baseline -eq "phase37") { "already GPU-accelerated Phase 37 runtime" } elseif ($Baseline -eq "phase36") { "already GPU-accelerated Phase 36 runtime" } elseif ($Baseline -eq "phase35") { "already GPU-accelerated Phase 35 runtime" } elseif ($Baseline -eq "phase34") { "already GPU-accelerated Phase 34 runtime" } else { "already GPU-accelerated Phase 17 runtime" }
}
$summary = [ordered]@{
