...
from .arithmetic_abi import sharrow15_cuda_reduction
...
def _compile_kernel(cp):
    ...
    if _KERNEL is not None:
        return _KERNEL
    reduction = sharrow15_cuda_reduction()
    source = r'''
    ...
    float max_bike_distance)
{
    ...
    // Generated by the versioned shared arithmetic ABI.  The association
    // matches the one-row OpenBLAS SGEMV fallback used by Sharrow's Numba
    // idotter: left-associated groups of four followed by three scalar terms.
    __CHOICEFORGE_REDUCTION__
...
'''.replace("__CHOICEFORGE_REDUCTION__", reduction)
